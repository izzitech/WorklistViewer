...
Well, this is a little tool that just query a Dicom Worklist and show the results.
...
The same thing with number fields.
...
Apparently it doesn't work well on XP on that res, so I would compile it twice: once with the 64px ".ico" icon, and after that redo it again with the full res for any other Windows version.

It's a super-ultra-mega-customized tool made for my job, using the RZDCX Dicom Toolkit, so it might not work on your environment.
